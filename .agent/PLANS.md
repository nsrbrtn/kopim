...
# ExecPlan: Группы категорий (UI + данные + синк)

## Context and Orientation
- Цель: добавить группы категорий (включая избранные), отображение сгруппированных категорий в управлении и при добавлении транзакций, а также экран редактирования группы.
- Область кода: `lib/features/categories/*`, `lib/features/transactions/presentation/widgets/transaction_form_view.dart`, `lib/core/data/database.dart`, `lib/core/services/auth_sync_service.dart`, `lib/core/services/sync/*`, `lib/core/navigation/app_router.dart`, `docs/*`.
- Контекст/ограничения: офлайн‑first через Drift; не править автоген (`*.g.dart`, `*.freezed.dart`, `*.drift.dart`); изменения схемы Drift только через миграции с планом отката.
- Риски: расхождение синка из‑за новой модели групп, регресс UI в выборе категорий, неверная обработка избранных/непривязанных категорий.

## Interfaces and Dependencies
- Внешние сервисы (Firebase/Analytics/Crashlytics): Firestore sync (если группы синкаются).
- Локальные зависимости (Drift, codegen): новая таблица(ы) групп и/или связи категории‑группы, миграции, build_runner.
- Затрагиваемые API/модули: CategoryRepository, CategoryDao, SyncService, providers категорий в формах транзакций.

## Plan of Work
- Спроектировать правила групп (избранные, множественная принадлежность, порядок).
- Добавить модель и хранение групп (Drift + domain + repositories + sync).
- Обновить UX управления категориями и экран добавления транзакции.
- Добавить тесты и документацию.

## Concrete Steps
1) Зафиксировать требования: как работает группа «Избранные», можно ли категории в нескольких группах, как показывать «Без группы», сортировка/раскрытие.
2) Спроектировать модель данных (таблица `category_groups` + связка или `group_id` в `categories`) и контракт доменного слоя (Freezed сущность/DTO).
3) Добавить Drift‑миграции, DAO, репозиторий/юзкейсы для CRUD групп и состава группы; обновить синк (Firestore payload и санитизацию).
4) UI управления категориями: кнопка «Добавить группу», список групп с раскрытием, иконка редактирования у заголовка; fallback на старый список если групп нет.
5) UI редактирования группы: имя + список категорий с добавлением/удалением; корректная валидация и сохранение.
6) UI добавления транзакции: отображение категорий по группам, по умолчанию свернуто, кроме «Избранные».
7) Тесты (unit: мапперы/репозитории, widget: списки групп/селектор), обновить `docs/components/` и `docs/logic/feature_invariants.md`.
...
```
- Acceptance criteria:
  - Группы создаются/редактируются и синкаются (если включён sync).
  - В управлении категориями отображаются группы с раскрытием и редактированием.
  - В форме транзакции категории показываются по группам, «Избранные» раскрыта по умолчанию.
  - При отсутствии групп UI ведёт себя как раньше.

## Idempotence and Recovery
- Что можно безопасно перезапускать: build_runner, тесты, миграции в тестовой БД.
- Как откатиться/восстановиться: удалить новые таблицы/поля, вернуть старые провайдеры списков категорий.
- План rollback (для миграций): миграция‑обратка + сохранение исходных данных групп в отдельной таблице/экспорте до cutover.

## Progress
- [x] Шаг 1: Уточнить требования и контракт групп.
- [x] Шаг 2: Спроектировать модель данных и API домена.
- [x] Шаг 3: Drift‑миграции + DAO/репозитории + синк.
- [x] Шаг 4: Экран управления категориями (группы + редактирование).
- [x] Шаг 5: Экран редактирования группы.
- [x] Шаг 6: Категории по группам в форме транзакции.
