...
- Первая дата платежа для нового кредита выбирается относительно текущей локальной даты:
  - если `paymentDay` еще не наступил в текущем месяце, используется текущий месяц;
  - если `paymentDay` уже прошел, используется следующий месяц;
  - если дня нет в месяце, используется последний день месяца.
