...
Приложение в целом имеет понятную feature-first структуру и во многих критичных модулях уже покрыто тестами, но на момент первичного аудита были найдены несколько серьезных проблем:
...
Актуализация на 2026-06-09:

- grouped credit feed переведен на модель без фиктивных `creditId: ''` и placeholder-валюты;
- drift warning о множественных `AppDatabase` в тестовом контуре устранен через корректные provider overrides;
- бессмысленный skipped placeholder-тест удален, а полезный narrow-layout тест analytics восстановлен;
- `BudgetDetailScreen` и `overview_goal_focus` не удалены окончательно: по ним принято временное решение оставить код как отложенный сценарий и пометить `TODO(ai-audit)` до отдельной продуктовой проверки.
...
### Частично закрыто 2026-03-07, уточнено 2026-06-09: часть dead code удалена, часть оставлена как отложенный сценарий
...
- удалены неиспользуемые `SwipeHintArrows` и `MainNavigationRail`;
- `BudgetDetailScreen`, `WatchOverviewGoalFocusUseCase`, `OverviewGoalFocus` и provider в DI не удаляются повторно:
  по состоянию на 2026-06-09 они сохранены в кодовой базе как отложенные сценарии без активных consumers и помечены `TODO(ai-audit)` до отдельной продуктовой верификации.

Статус:

- находка закрыта частично;
- подтверждение: `flutter analyze` и `flutter test --reporter expanded` проходят, а решение по двум возвращенным кандидатам зафиксировано в коде и документации.
