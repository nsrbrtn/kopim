...
Важно:
- язык интерфейса влияет только на разделители, порядок записи и локализованные подписи;
- символ валюты берется из явного `currency` (например, `RUB`, `USD`, `EUR`), а не из текущей локали интерфейса;
- переключение `ru -> en` не должно менять `₽` на `$`, если валюта пользователя или счета остается `RUB`.
- то же правило действует для AI-контекста и prompt-building: локаль влияет на текстовые даты и percent-pattern, но не должна подменять currency symbol в суммах.
