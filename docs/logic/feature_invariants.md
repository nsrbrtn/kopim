...
- Категории могут входить в несколько групп; связи хранятся в `category_group_links`.
- Группа «Избранные» формируется автоматически по `isFavorite`.
- Категории без групп отображаются в группе «Остальное».
- Порядок групп хранится в `sortOrder`, переупорядочивание — через массовое обновление.
