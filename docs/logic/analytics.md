...
1. **Bar chart категорий**: используется для отображения структуры расходов и доходов по категориям в выбранном месяце.
...
Особенности UX карточки «Операции по категориям»:
- переключатель `Расходы / Доходы` выполнен как текстовые табы без фоновых чипов;
- круговая диаграмма убрана; на карточке остается только bar chart;
- выбранная категория сохраняется при переключении между месяцами, чтобы можно было смотреть один и тот же расходный сегмент в динамике.

### Операции по категориям
- Карточка «Операции по категориям» поддерживает переключение режима между расходами и доходами.
- В режиме расходов доступен чип «Показывать переводы», который включает/исключает из графика:
  - переводы между обычными счетами;
  - платежи по кредитам и долгам (агрегированные credit/debt outflow-операции).
- Переводы, затрагивающие liability-счета (`credit`, `credit_card`, `debt`), не попадают в сегмент «Переводы» и отображаются только в сегменте кредитно-долговых операций.
- Для legacy-кредитов аналитика дополнительно ориентируется на связь `CreditEntity.accountId`, даже если raw `accounts.type` уже не нормализуется как liability.

### Вкладка «Кредиты»
- Во вкладке отображаются два виджета:
  - **Общая сумма долга** — суммарный долг по liability-счетам (`credit`, `credit_card`, `debt`, включая `custom:`-типы).
  - **Динамика долга** — линейный график суммарного долга за последние 6 месяцев.
- Месячная динамика рассчитывается по `openingBalance` и транзакциям (`income`, `expense`, `transfer`) для liability-счетов.
