...
- Примечание: при фильтре по категории учитываются дочерние категории.
...
### 7) get_budgets
- Назначение: бюджеты с прогрессом, остатком и разбивкой по категориям.
- Параметры: `budget_id`, `category_limit`.
- Ограничения: category_limit до 50.
