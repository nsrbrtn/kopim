...
class AccountTypeBackfillService {
  ...
  Future<AccountTypeBackfillResult> run({
    ...
    final _BackfillRunOutput output = await _migrationWriteGuard
        .runBackgroundMutation(
          action: () async {
            final List<AccountEntity> accounts = await _accountDao
                .getAllAccounts();
            ...
            for (final AccountEntity account in accounts) {
              final String? canonicalType = resolveBackfillAccountType(
                account.type,
              );
            ...
            return _BackfillRunOutput(
              result: AccountTypeBackfillResult(
                scannedCount: accounts.length,
                updatedCount: updatedCount,
                skippedUnknownCount: skippedUnknownCount,
                skippedUpToDateCount: skippedUpToDateCount,
              ),
              unknownLegacyTypes: unknownLegacyTypes,
            );
          },
        );
    final AccountTypeBackfillResult result = output.result;
    if (output.unknownLegacyTypes.isNotEmpty) {
      _logger?.logInfo(
        'AccountTypeBackfillService: skipped unknown legacy account types: '
        '${output.unknownLegacyTypes.join(', ')}',
      );
    }
    _logger?.logInfo(
    ...
    if (_analytics != null) {
      await _analytics
          .logEvent('account_type_backfill_completed', <String, dynamic>{
            ...
            'unknownTypeCount': output.unknownLegacyTypes.length,
          });
    }
    return result;
  }
...
class _BackfillRunOutput {
  const _BackfillRunOutput({
    required this.result,
    required this.unknownLegacyTypes,
  });

  final AccountTypeBackfillResult result;
  final Set<String> unknownLegacyTypes;
}
