...
class GoalContributionRebuildService {
  GoalContributionRebuildService(this._database);
    ...
    await _database.transaction(() async {
      ...
      final List<db.TransactionRow> activeTransactions =
          await (_database.select(_database.transactions)..where(
                (db.$TransactionsTable tbl) =>
                    tbl.isDeleted.equals(false) & tbl.savingGoalId.isNotNull(),
              ))
              .get();
        ...
        await _database
            ...
            .insertOnConflictUpdate(
              db.GoalContributionsCompanion(
                ...
                amount: Value<int>(BigInt.parse(row.amountMinor).abs().toInt()),
      ...
      for (final db.GoalContributionRow row in existingRows) {
        ...
        await (_database.delete(
            ...
            )..where((db.$GoalContributionsTable tbl) => tbl.id.equals(row.id)))
            .go();
      }
      ...
      for (final db.SavingGoalRow row in goalRows) {
        ...
        )..where((db.$SavingGoalsTable tbl) => tbl.id.equals(row.id))).write(
          db.SavingGoalsCompanion(currentAmount: Value<int>(recalculated)),
        );
      }
    });
  }
