      ...
      data: (DataModeState dataModeState) {
        switch (featureAccess.cloudSync.status) {
          ...
          case FeatureAccessStatus.blockedByLocalData:
            final AuthUser? blockedUser = ref
            ...
            if (blockedUser != null) {
              return Column(
                ...
                children: <Widget>[
                  ...
                  const SizedBox(height: 12),
                  Text(
                    'Вход в облачный аккаунт выполнен, но перед включением синхронизации нужно отдельно решить, что делать с локальными данными на устройстве.',
                    style: theme.textTheme.bodyMedium,
                  ),
                  const SizedBox(height: 16),
                  ElevatedButton(
          ...
          case FeatureAccessStatus.unavailable:
            return Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: <Widget>[
                Row(
                  children: <Widget>[
                    Icon(Icons.cloud_sync_outlined, color: iconColor),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Text(
                        'Проверяем доступ к синхронизации',
                        style: theme.textTheme.titleMedium,
                      ),
                    ),
                  ],
          ...
          case FeatureAccessStatus.disabledByBuild:
            return const SizedBox.shrink();
          case FeatureAccessStatus.enabled:
            break;
        }
