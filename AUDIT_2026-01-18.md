...
- `flutter test --reporter expanded` — FAIL (таймаут 300s)
  - Зафиксированы падения тестов, см. раздел 5
...
## 5) Тесты
**Запуск:** `flutter test --reporter expanded` (2 попытки, таймаут 300s). Набор тестов не завершился полностью.

**Падения/ошибки (из вывода):**
...
**Транзакции:**
- Инварианты денег нарушаются из‑за `double` и двойного пересчёта балансов.
- Обновление/удаление транзакций несогласовано между use cases и репозиторием.
- Переводы между счетами в аналитике не учитываются корректно.

**Аналитика:**
- Агрегации по всем транзакциям на стороне UI → риск производительности.
- Баланс/кэшфлоу не учитывают перевод на целевой счёт (ошибка аналитики).
...
- 2026-01-18: баланс пересчитывается только в use cases (удалён пересчёт из `TransactionRepositoryImpl`), добавлен unit‑тест на смену типа транзакции.
...
- `lib/core/services/auth_sync_service.dart:765` — запись `TransactionDao.upsertAll` при sync.  
  Исправление: добавлен пересчёт балансов по списку транзакций и апдейт аккаунтов после записи.  
  Примечание: логика считает баланс от 0 по транзакциям (без учёта стартового баланса).
- `lib/features/settings/data/repositories/import_data_repository_impl.dart:35` — запись `TransactionDao.upsertAll` при импорте.  
  Исправление: добавлен пересчёт балансов после импорта транзакций + unit‑тест.  
...
- `lib/features/savings/data/repositories/saving_goal_repository_impl.dart:226` — запись `TransactionDao.upsert` при взносах.  
  Риск минимальный, т.к. баланс обновляется вручную в этом же методе.  
  Путь исправления (если хотите унификацию): вынести создание транзакции в `AddTransactionUseCase` и положиться на общую логику баланса.
