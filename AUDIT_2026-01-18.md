...
- Пункт про UTC vs local — закрыт (нормализация timestamps в UTC).
- Пункт 6 — закрыт: Phosphor‑шрифты подключены в `pubspec.yaml`.
- Пункт 7 — закрыт: задокументирован `--flavor` и обновлён perf‑скрипт.
...
- Закрыто: UTC vs local (timestamps транзакций нормализованы в UTC).
- Закрыто: web‑шрифты Phosphor (явное подключение шрифтов).
- Закрыто: Android APK/flavor (добавлены команды с `--flavor`).
