...
- Пункт 3 — частично закрыт: внедрены `minor/scale`, `double` остаётся как legacy/UI‑слой.
...
- Закрыто: двойной пересчёт баланса, `softDelete` баланс, LWW merge без пересчёта, учёт переводов в аналитике.
- Частично: переход с `double` на `minor/scale` (legacy‑поля остаются).
